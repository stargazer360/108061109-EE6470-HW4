    ...
    while (true) {
      ...
      for (unsigned int v = 0; v < MASK_Y; ++v) {
        for (unsigned int u = 0; u < MASK_X; ++u) {
          r += (unsigned int)i_r.read() << filter[v][u];
          g += (unsigned int)i_g.read() << filter[v][u];
          b += (unsigned int)i_b.read() << filter[v][u];
          wait(CLOCK_PERIOD, SC_NS);
        }
      }
      unsigned char out_r = static_cast<unsigned char>(r >> 4);
      unsigned char out_g = static_cast<unsigned char>(g >> 4);
      unsigned char out_b = static_cast<unsigned char>(b >> 4);
      wait(CLOCK_PERIOD, SC_NS);
      o_r.write(out_r);
      o_g.write(out_g);
      o_b.write(out_b);
