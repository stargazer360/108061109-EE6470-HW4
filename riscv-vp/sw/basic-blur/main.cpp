...
int main(int argc, char *argv[]) {
  ...
  for (int y = 0; y != height; ++y) {
    for (int x = 0; x != width; ++x) {
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = buffer[0];
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = buffer[1];
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = buffer[2];
    }
  }
